...
export default function MakePostPage() {
  ...
  async function handleSubmit(e) {
    ...
    if (images.length > 0) payload.post_image_ids = images.map((img) => img.id);

    try {
      const { ok, body } = await createPost(payload);

      if (ok) {
        navigate(`/posts/${body.data.id}`, { replace: true });
        return;
      }
  ...
  return (
    <main className="flex flex-col items-center justify-center py-14">
      <h2 className="text-title text-ink">게시글 작성</h2>
